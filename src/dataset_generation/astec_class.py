...
class Astec_Dataset():
    ...
    def build_training_dataset(self, indeces, purpose_of_data):
        ...
        self.dictionary_per_time_lengths = extract_input_output_bc_variables(self.path_to_hdf5, indeces, False) #build dictionary of data divided by lenghts of time series
        self.dictionary_per_time_lengths = self.make_channels_for_dictionary_per_time_lengths(self.dictionary_per_time_lengths) #build dictionary of data divided by lenghts of time series and make channels per spatial domain
        ...
        if self.save_dictionary_per_time_lengths:
            with h5py.File(self.path_to_hdf5+'/data_'+self.purpose_of_data+'.h5', 'w') as f:
                dict_to_hdf5(self.dictionary_per_time_lengths, f) 
        ...
        #divide datasets in time windows of length t_W and mix the trajectories
        ...
        #normalize dictionary with sliced windows
    ...
    def make_channels_for_dictionary_per_time_lengths(self, dict:dict):
        for l_t_s in dict: #l_t_s is length time series
            for m_t in dict[l_t_s]: #m_t is the mesh type (so 1, 76, 32 and so on )
                fields = list(dict[l_t_s][m_t].keys())
                size_field = len(np.shape(dict[l_t_s][m_t][fields[0]])) #final [0] to take first batch in case there are multiple trajectories with the same time lenght
                if  size_field == 3:
                    concatenated_array = np.concatenate([np.array(dict[l_t_s][m_t][field])[:,:,None,:] for field in dict[l_t_s][m_t]], axis = 2)
                elif size_field == 2:
                    concatenated_array = np.concatenate([np.array(dict[l_t_s][m_t][field])[:,:, None] for field in dict[l_t_s][m_t]], axis = -1)
                else:
                    raise TypeError("Something is wrong with data structure")   
                 
                dict[l_t_s][m_t] = concatenated_array
        # now mix the bcs
        for i in dict:
    ...
    def make_dictionary_unified(self):
        keys = list(self.dictionary_per_time_lengths.keys())
        dictionary_unified = {}
        variables = self.dictionary_per_time_lengths[keys[0]].keys()
        for key in keys:
            for x in variables:
                size = np.shape(self.dictionary_per_time_lengths[key][x])
                reshaped_field = np.reshape(self.dictionary_per_time_lengths[key][x], (size[0] * size[1],)+size[2:])
                dictionary_unified[x] = reshaped_field
    ...
    def make_dictionary_of_sliced_windows(self):
        ...
        variables = self.dictionary_per_time_lengths[keys[0]].keys()
        for x in variables:
            size = (1, self.t_W) + np.shape(self.dictionary_per_time_lengths[keys[0]][x])[2:] 
            dictionary_of_sliced_windows[x] = tc.tensor([])
        dictionary_of_sliced_windows['length_of_padding'] = tc.tensor([])
        for key in keys:
            for x in variables:
                size = np.shape(self.dictionary_per_time_lengths[key][x])
                pad_to_be_added = (self.t_W - (size[1]%self.t_W)) % self.t_W
                padded_tensor = tc.nn.functional.pad(tc.tensor(np.array(self.dictionary_per_time_lengths[key][x])), (0, 0)*(len(size)-2) + (0, pad_to_be_added))
                padded_tensor = tc.reshape(padded_tensor, (size[0] * int((size[1]+pad_to_be_added)/self.t_W), self.t_W) + size[2:])
                dictionary_of_sliced_windows[x] = tc.concatenate((dictionary_of_sliced_windows[x], padded_tensor), axis = 0)
    ...
    def normalize_dictionary_of_sliced_windows(self):
        ...
        normalized_dict = {}
        keys = self.dictionary_of_sliced_windows.keys()
        for key in keys:
            if key != 'length_of_padding':
                original_data = self.dictionary_of_sliced_windows[key][:]
                ...
                normalized_dict[key] = self.dictionary_of_sliced_windows[key]
