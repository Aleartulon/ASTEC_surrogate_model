...
class Training():
    ...
    def train_epoch(self, loss_coefficients):
        ...
        for fields, boundary_conditions_and_time in self.training_loader:
            
            l1 = self.training_losses.loss_sup_mixed(fields, boundary_conditions_and_time, loss_coefficients, True)
            l1_mean = l1[0]
        ...
        return l1_loss/count, l1_loss_per_shape/count
    ...
    def valid_epoch(self, loss_coefficients):
        ...
        with tc.no_grad():
            for fields, boundary_conditions_and_time in self.validation_loader:
                l1 = self.training_losses.loss_sup_mixed(fields, boundary_conditions_and_time, loss_coefficients, False)
                l1_mean = l1[0]
                l1_mean_per_shape = l1[1]
                ...
                count += 1
        return l1_loss/count, l1_loss_per_shape/count, l1_loss_unnorm/count, l1_loss_unnorm_per_variable/count
    ...
    def training(self):
        ...
        if not self.checkpoint:
            ...
            for i in range(self.epochs):
                ...
                if np.mean(valid_l1_data) < loss_value: #careful valid loss tot!!
                    loss_value = np.mean(valid_l1_data)
                    print('Models saved!')
                    save_checkpoint( self.decoder, self.optim, self.scheduler, i, loss_value, full_training_count, self.PATH+'/checkpoint/check.pt')
                    early_stopping = 0
            ...
            for i in np.arange(start_epoch+1, self.epochs+1, 1):
                ...
                elif i >=self.warm_up_time and i < time_only_TF: #use only TF
                ...
                #check if training a noncoupled system and adjust accordingly the validatin losses to be checked for early stopping
                if not self.is_coupled[0] and self.is_coupled[1] == 'AE' and i >=self.warm_up_time:
